...
class Cart extends Model {
    ...
    public function getProducts()
    {
        ...
        $rows = $sql->select("SELECT b.idproduct, b.desproduct, b.vlprice, 
            b.vlwidth, b.vlheight, b.vllength, b.vlweight, b.desurl, COUNT(*) AS nrqtd,
    ...
    public function getProductsTotals()
    {
        ...
        $results = $sql->select("SELECT SUM(vlprice) AS vlprice, SUM(vlwidth) AS vlwidth, 
            SUM(vlheight) AS vlheight, SUM(vllength) AS vllength, SUM(vlweight) AS vlweight,
            COUNT(*) AS nrqtd
            FROM tb_products a
    ...
    public function setFreight($nrzipcode)
    {
        ...
        if ($totals['nrqtd'] > 0) 
        {
            //rule from post office in Brazil
            if ($totals['vlheight'] < 2) $totals['vlheight'] = 2;
            //rule from post office in Brazil
            if ($totals['vllength'] < 16) $totals['vllength'] = 16;

            //builds query from array
            //every key is needed, even if empty
            $qs = http_build_query([
                'nCdEmpresa'=>'',
                'sDsSenha'=>'',
                'nCdServico'=>'40010',
            ...
            $this->setvlfreight(Cart::setValueToDecimal($result->Valor));
    ...
    //changes decimal numbers from BR format to PHP format
    public static function setValueToDecimal($value):float 
    {
